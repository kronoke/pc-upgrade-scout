(function(){
  ...
  if(fieldGrid&&!form.querySelector('.bios-guide-inline')){
    ...
    note.className='bios-guide-inline';
    note.innerHTML='<strong>CPU fits the socket—but will it boot?</strong><span>Some motherboard and CPU combinations require a BIOS update first.</span><a href="cpu-motherboard-bios-compatibility-guide.html">Learn how to check BIOS support safely →</a>';
    fieldGrid.insertAdjacentElement('afterend',note);
  }

  const biosScript=document.createElement('script');
  biosScript.src='calculator-bios-support.js';
  biosScript.defer=true;
  document.head.appendChild(biosScript);
